...
interface PurchaseInvoice { id: string; supplierId: string; warehouseId: string; invoiceNumber: string; date: string; supplierName: string; items: { id: string; name: string; qty: number; price: number; }[] }
    ...
    <>
        <div className="grid md:grid-cols-3 gap-6">
            <div className="space-y-2">
                ...
                <Select value={supplierId} onValueChange={setSupplierId} disabled>
          ...
          <CardContent className="space-y-6">
            {loading ? (
                ...
                <div className="space-y-4">
                    <Label htmlFor="invoice-select">اختر فاتورة الشراء الأصلية</Label>
                        ...
                        <SelectContent>
                            {invoices.map(inv => (
                                <SelectItem key={inv.id} value={inv.id}>
                                    {`${inv.invoiceNumber} - ${inv.supplierName} - (${new Date(inv.date).toLocaleDateString('ar-EG')})`}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
